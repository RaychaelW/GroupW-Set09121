    ...
    : manager(manager)
{
    ...
    gridImage.create(800, 600, sf::Color::Transparent);
    for (int y = 0; y < 600; ++y) {
        for (int x = 0; x < 800; ++x) {
            if (x % 40 == 0 || y % 40 == 0) {
...
void SettingsState::render(sf::RenderWindow& window)
{
    ...
    sf::RectangleShape header(sf::Vector2f(800, 120));
    header.setFillColor(sf::Color(20, 25, 45, 180));
    window.draw(header);

    // Draw decorative footer
    sf::RectangleShape footer(sf::Vector2f(800, 60));
    ...
    for (int i = 0; i < options.size(); i++) {
        ...
        sf::RectangleShape optionBg(sf::Vector2f(600, 45));
