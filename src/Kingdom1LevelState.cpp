    ...
    : manager(manager), levelNumber(levelNumber), kingdom(kingdom), level(level)
{
    ...
    enemyTexture = resources.getTexture("resources/tilesets/Sprites/Enemies/Default/slime_normal_walk_a.png");
...
void Kingdom1LevelState::loadLevel() {
    ...
    if (kingdom == KingdomID::Kingdom1) {
        switch (level) {
            case LevelID::Level1: map.load("resources/tilemaps/lvl1.tmx");
                break;
            case LevelID::Level2: map.load("resources/tilemaps/lvl2.tmx");
                break;
            case LevelID::Level3: map.load("resources/tilemaps/lvl3.tmx");
                break;
        }
    }
...
void Kingdom1LevelState::loadEnemies() {
    ...
    for (const auto& spawn : map.getEnemies()) {
        ...
        Enemy enemy(type, KingdomTheme::Kingdom1, spawn.bounds.left, spawn.bounds.top);
